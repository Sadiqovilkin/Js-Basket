...
window.addEventListener("load", () => {
  ...
  if (!localStorage.getItem("whishtlist")) {
    localStorage.setItem("whishtlist", JSON.stringify([]));
  }
});

data.products.forEach((product) => {
  const whishtlist = JSON.parse(localStorage.getItem("whishtlist"))
  const currentWhist = whishtlist.find((x)=> x.id == product.id)

  cardsRow.innerHTML += `  <div class="col-lg-3 col-md-6 col-sm-12">
    <div class="card">
        <div class="heart"><i class="${currentWhist ? 'fa-solid' : 'fa-regular'} fa-heart whistListBtn" data-id='${product.id}' style="color: #ff0000;"></i></div>
  ...
  const whisListBtns = document.querySelectorAll('.whistListBtn')
  whisListBtns.forEach(heartBtn => {
    heartBtn.addEventListener('click',function(e){
      let localWhistArr = JSON.parse(localStorage.getItem("whishtlist")) || [];
      let currentProID = this.getAttribute("data-id");
      let founDuplicate = data.products.find((x) => x.id == currentProID);
      if (localWhistArr.some(item => item.id === founDuplicate.id)) {
        // console.log(founDuplicate.whistlisted);
        // heartBtn.classList.replace('fa-regular','fa-solid')
        // founDuplicate.whistlisted = true
        // data.products.push(founDuplicate)
        e.target.classList.replace('fa-solid','fa-regular')
        localWhistArr = localWhistArr.filter(item =>item.id !== founDuplicate.id)
        localStorage.setItem("whishtlist" , JSON.stringify(localWhistArr))
      }
      else{
        // console.log(founDuplicate.whistlisted);
        // founDuplicate.whistlisted = false
        // data.products.push(founDuplicate)
        // heartBtn.classList.replace('fa-solid','fa-regular')
        e.target.classList.replace('fa-regular','fa-solid')
        localWhistArr.push(founDuplicate)
        localStorage.setItem("whishtlist" , JSON.stringify(localWhistArr))
      }
    })
  });
